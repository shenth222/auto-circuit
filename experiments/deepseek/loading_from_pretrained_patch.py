...
from HookedTransformerConfig_patch import HookedTransformerConfig_patch as HookedTransformerConfig
...
def get_official_model_name(model_name: str):
    ...
    if os.path.exists(model_name):
        print(f"Model name {model_name} is a path to a local model.")
        return model_name
...
def convert_hf_model_config_patch(model_name: str, **kwargs):
    """
    ...
    if official_model_name.startswith(
        ...
        cfg_dict = {
            "d_model": hf_config.hidden_size,
            "d_head": hf_config.hidden_size // hf_config.num_key_value_heads,
            "n_heads": hf_config.num_attention_heads,
            "d_mlp": hf_config.intermediate_size,
            "n_layers": hf_config.num_hidden_layers,
            "n_ctx": hf_config.max_position_embeddings,
            ...
            "use_local_attn": False,
            "normalization_type": "RMS",
            "aux_loss_alpha": hf_config.aux_loss_alpha,
...
def get_pretrained_model_config_patch(
    model_name: str,
    ...
    **kwargs,
    ...
    if (
        ...
        cfg_dict = convert_hf_model_config_patch(official_model_name, **kwargs)
...
def convert_deepseek_weights(deepseek, cfg: HookedTransformerConfig):
    ...
    # dense layers, determined by "config.first_k_dense_replace"
    for l in range(cfg.n_layers):
        # input_layernorm
        state_dict[f"blocks.{l}.ln1.w"] = deepseek.model.layers[l].input_layernorm.weight
        # attn
        # (num_heads, q_head_dim), hidden_size -> num_heads, hidden_size, q_head_dim
        W_Q = deepseek.model.layers[l].self_attn.q_proj.weight
        W_Q = einops.rearrange(W_Q, "(n h) m->n m h", n=cfg.n_heads)
        W_QN, W_QR = torch.split(
            W_Q,
            [cfg.qk_nope_head_dim, cfg.qk_rope_head_dim],
            dim=-1
        )
        state_dict[f"blocks.{l}.attn.W_QN"] = W_QN

        # kv_lora_rank + qk_rope_head_dim, hidden_size -> hidden_size, kv_lora_rank + qk_rope_head_dim
        W_KV_A = deepseek.model.layers[l].self_attn.kv_a_proj_with_mqa.weight.T 
        # kv_a_layernorm
